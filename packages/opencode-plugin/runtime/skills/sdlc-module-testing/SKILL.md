...
若该模块已有已批准通过的模块测试版本，且当前总设计和模块设计版本与配方记录一致，优先只调用一次
`sdlc_test_execute_existing(scopeType=MODULE,moduleName=<完整模块名称>,createCandidate=true)`。该工具从批准版本
恢复原命令配方，以当前代码和设计版本建立新运行，依次执行命令、形成记录，并仅在全部通过时创建候选；
工具内部不调用模型。不得先读取完整旧记录再逐步重构运行参数。若没有批准配方或测试设计已经变化，才执行
下述首次测试流程并重新确认命令。
...
同一运行中的同一命令最多执行两次。第一次失败后只允许在根因已经明确处理时原样重试一次；第二次仍失败，
插件会阻止第三次执行，必须结束运行并返回诊断，不得继续循环消耗 token。

持续集成或人工只需重跑已有测试时，直接在目标项目执行：

```powershell
node .opencode\bin\sdlc-test-run.mjs module --module "<完整模块名称>"
```

该入口不启动 OpenCode 模型，默认只形成运行和测试记录；需要进入版本审核时显式增加 `--candidate`。
