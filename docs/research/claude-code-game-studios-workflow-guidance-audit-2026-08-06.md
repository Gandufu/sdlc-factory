...
Factory 不需要引入对话模型、消息流程上下文、流程绑定、统一命令拦截器或通用工作流引擎。只需让每个 `/sdlc-*` Skill 在开始工作前读取 Core 已有的阶段、产物和审核事实，再由 AI 判断是否需要提醒：
...
尚未完成：
- CU-03 设计未审核 → 建议 /sdlc-review CU-03
- 总体设计仍有 2 个未决项 → 建议 /sdlc-design

你正在执行：/sdlc-code CU-03
继续执行可能基于未确认设计产生返工。

你可以选择继续执行，也可以先运行上面的建议命令。
```

这里的“尚未完成”是 AI 根据 Core 事实给出的建议信息，不是 Core Gate 拒绝，也不需要单独设计产品拦截卡。
...
- 不照搬“AI 扫 Markdown 推断一切”的非确定性实现；Factory 可以让 Core 读取 SQLite 中的阶段事实，并辅以工作区产物检测；
