...
- 系统验收使用专用确定性入口，只绑定当前系统测试版本、同一报告和同一批测试记录，不重复运行测试；
- 状态查询实时校验已批准代码和测试指纹，代码漂移会向依赖模块、模块测试、系统测试和系统验收传播；
- 命令、Skills、压力场景和自动化测试已经按新合同重写。

真实目标项目已经完成两个业务模块从需求、设计、编码、模块测试到系统验收的完整版本链，执行 30 项项目测试、完整应用打包及 Playwright 跨模块流程，并生成环境、实际接口地址、命令证据、测试记录和自动报告。全新进程和迁移副本均能恢复最终状态；真实代码漂移会使两个相关模块及系统测试、系统验收失效，失效状态下系统测试被确定性阻止。候选快照已经改为内容寻址复用，模块测试和系统测试候选已经改为专用确定性入口。第 15 节验收条件已经满足。
