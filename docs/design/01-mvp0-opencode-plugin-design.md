...
SDLC Factory 使用 OpenCode 项目级插件、命令和 Skills 实现以下完整闭环：
...
产品只交付目标项目内的 OpenCode 插件能力，不引入独立服务端、通用工作流引擎或旧的能力单元模型。
...
`.sdlc-factory` 保存结构化项目事实；`docs` 保存用户可读正文；`evidence` 保存执行输出。用户不需要直接编辑 `.sdlc-factory`，模型也不得扫描该目录。

候选、审核、已批准版本、运行、命令证据和测试记录分别保存为单一职责的不可变 JSON。文件数量等于事实数量，不应为了减少文件数而合并为可变大文件。候选正文和代码快照存入 `objects/sha256`，相同字节只保存一次；详细理由见 [DESIGN-04《插件产物与质量设计》](./04-plugin-artifact-quality-design.md)。
...
状态服务同时返回当前唯一目标的对象种类、范围、规范文档路径和父版本。没有命令参数时，需求和设计写入以及
候选创建必须严格等于该目标，不能在生成需求地图时顺带改写已批准产品概述。需要修订既有对象时，用户必须在
命令中写出完整对象名称；插件再根据已批准需求地图解析该对象，只开放它自己的规范路径。这样既防止模型扩大
当前工作范围，也保留需求和设计的正常版本修订能力。
...
安全审核投影还必须包含候选的结构化事实：需求地图展示业务模块、功能组、执行依赖、外部接口和非功能需求引用，
模块设计展示产品代码与测试代码路径边界。用户因此能够在批准前发现把页面误当成业务模块等语义问题。
...
7. 生成结构化测试记录和用户可读报告；
8. 等待人工审核并形成系统测试版本；
9. 系统测试版本批准后，由专用确定性工具复用同一报告和测试记录形成系统验收候选；
10. 等待人工审核系统验收。

Playwright 不执行单元测试。测试代码必须预先存在，系统测试阶段不实时要求 AI 重新生成全套测试。

首次测试由用户确认实际命令和环境；形成已批准通过的测试版本后，插件从其命令证据投影不可变配方，以当前
版本一次调用重跑。自动化使用 `.opencode/bin/sdlc-test-run.mjs`，不启动模型；命令配方不形成新的实施计划
或业务版本。系统环境每次显式指定，正常通过后也只能形成待人工审核候选。
...
`packages/opencode-plugin` 已完成以下完整合同：

- 使用业务模块、需求地图、候选、审核和已批准版本替换旧执行单位；
- 候选固定规范化文件清单、不可变字节快照、父版本、输入版本、来源和内容哈希；
- 人工审核绑定当前会话用户原文、候选编号和完整 SHA-256；
- 总需求和总设计版本由系统根据当前已批准组件生成；
- 项目进度由版本、候选、审核、运行、测试记录和事件实时推导，不保存独立副本；
- 编码运行强制使用 OpenCode 原生待办，并在工具层阻止待办建立前的修改和命令；
- 受控执行器自动保存退出码、耗时、脱敏输出和证据哈希；
- 环境实际地址、凭据引用、测试记录和自动测试报告分别保存；
- 模拟、真实和未明确环境由不可变环境版本区分；只有真实环境通过记录可以进入系统验收；
- 系统验收使用专用确定性入口，只绑定当前真实系统测试版本、同一报告和同一批测试记录，不重复运行测试；
- 状态查询实时校验已批准代码和测试指纹，代码漂移会向依赖模块、模块测试、系统测试和系统验收传播；
- 命令、Skills、压力场景和自动化测试已经按新合同重写。

当前直升机会控目标项目已经完成两个业务模块从需求、设计、编码、模块测试到系统模拟测试的版本链，
真实执行完整应用打包和 Electron/Playwright 跨模块流程，并生成环境、地址、命令证据、测试记录和自动报告。
修复后的状态为“本地模拟闭环已验证”，不会把模拟记录冒充正式验收；真实设备密码和 TLS 信任方式具备后，
再按同一合同执行真实系统测试和系统验收。真实环境验收正路径、模拟阻断、漂移传播和重启恢复均有确定性测试覆盖。
