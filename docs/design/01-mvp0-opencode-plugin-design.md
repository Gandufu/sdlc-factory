...
`packages/opencode-plugin` 已完成以下核心合同：
...
当前自动化测试已证明双业务模块完整状态可以在重启后重建；真实目标项目已证明安装与升级、旧技能清理、初始化、来源、Skill、文档校验、候选安全投影、直接审核和不可变版本链可用。剩余验收是基于真实产品脚手架完成两模块编码、项目原生测试和 Playwright 跨模块流程。在该验收完成前，不启动桌面工作台实现。
